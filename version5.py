...
import threading
import time
from ocr_plate import plate_ocr  # Make sure your OCR module is available
...
def run_ocr_async(image_path):
    
    global ocr_running
    try:
        print("Processing OCR in background thread...")
        start_time = time.time()
        format=plate_ocr(image_path)
        print("Your number plate is:", format)
        end_time = time.time()
...
def process_frame(frame):
    ...
    """
    Process a single frame: run detection, draw bbox, store crops, handle stability.
    Returns:
        frame: annotated frame
        bbox: detected bbox or None
        best_crop: best crop for OCR or None
        sharpness: sharpness of best crop
    """

    global stable_count, last_bbox, crop_buffer
    ...
    return frame, bbox, best_crop, sharpness
...
try:
    ...
    while True:
        ...
        frame, bbox, best_crop, sharpness = process_frame(frame)
 
        if cooldown > 0:
            cooldown -= 1
 
        if stable_count >= STABLE_FRAMES and cooldown == 0 and best_crop is not None and not ocr_running:
            if sharpness >= MIN_SHARPNESS:
                clear_folder(raw_img_path)
                # Resize crop for PaddleOCR
                resized_crop = enhance_for_ocr(best_crop)
                out_path = os.path.join(raw_img_path, "plate_latest.png")
                cv2.imwrite(out_path, resized_crop)
                ocr_running = True
                threading.Thread(target=run_ocr_async, args=(raw_img_path,), daemon=True).start()
